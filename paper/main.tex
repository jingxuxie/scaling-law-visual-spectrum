...
\end{quote}
We answer in power-law Gaussian linear regression.  The setting is tractable enough for sharp proofs, yet retains the ingredients of contemporary scaling-law theory: approximation error, optimization bias, effective dimension, source conditions, feature maps, and compute allocation \cite{bartlett2020benign,hastie2022surprises,bordelon2020spectrum,canatar2021spectral,lin2024scaling}.  It also provides a controlled bridge to sketched models and to random-feature regression, where optimizer-induced exponent shifts have recently been observed empirically \cite{ramani2026optimizer}.
...
\caption{Visible-spectrum mechanism. Coordinatewise second moments track $\diag(\Sigma)$ up to a residual-dependent scalar and constant factors. Aligned or band-limited features expose the spectral profile, whereas globally mixed features with concentrated coordinate variances yield an approximately scalar preconditioner. The visible exponent then controls learned modes, risk, compute allocation, and AdamW decay.}
\label{fig:overview}
\end{figure*}
...
\midrule
\shortstack[l]{Second\\moment} &
\shortstack[l]{spectral\\preconditioner $P_t$} & determines $\qeff$\\
Momentum & stable temporal filter &
\shortstack[l]{constants/stability;\\exponent unchanged}\\
\shortstack[l]{Weight\\decay} &
\shortstack[l]{$\mu_i\gtrsim\delta$ threshold;\\$K\lesssim\delta^{-1/\alpha}$} &
\shortstack[l]{may lower exponent\\unless $\delta$ decays}\\
\bottomrule
\end{tabular}
\end{table}
...
The same population eigenspectrum can be visible or hidden depending on feature coordinates.  For comparison, a synthetic controlled profile can impose a target preconditioner directly.  Table~\ref{tab:visibility-cases} separates these settings.

\begin{table*}[t]
\centering
\small
\caption{Settings producing different visible-spectrum exponents.}
\label{tab:visibility-cases}
\begin{tabular}{@{}p{0.17\textwidth}p{0.29\textwidth}p{0.28\textwidth}p{0.12\textwidth}@{}}
\toprule
Setting & Visible structure & Effective preconditioner &
\shortstack{Predicted\\$\qeff$}\\
\midrule
Eigenbasis aligned & $\Sigma_{jj}=\lambda_j$ &
$(\Sigma+\rho I)^{-1/2}$ & $1/2$\\
Band-limited rotation & coordinate variances reveal band scale $\Lambda_\ell$ &
spectrally comparable, blockwise & $1/2$\\
\shortstack[l]{Synthetic controlled\\profile} & $P_\theta$ is imposed directly &
$(\Sigma^\theta+\rho I)^{-1/2}$ & $\theta/2$\\
Flat/Hadamard & $\Sigma_{jj}=\tr(\Sigma)/M$ &
scalar & $0$\\
\shortstack[l]{Haar/global\\Gaussian sketch} &
concentrated coordinate quadratic forms &
scalar up to constants & \shortstack[l]{$\approx0$ under\\concentration}\\
\bottomrule
\end{tabular}
\end{table*}
...
\caption{Empirical spectral visibility. Left: as a controlled-profile sanity check, constructed preconditioners satisfy $\qeff=\theta/2$; the categorical inset reports independent feature-system measurements. Center: tuned stochastic excess risk decreases as the controlled visible exponent increases. Right: after a wide learning-rate sweep, the best coordinatewise adaptive method (Adam for Gaussian sketch; AdamW for Haar) remains near $\qeff=0$, while a spectral oracle retains $\qeff=1/2$ and achieves lower risk.}
...
\toprule
$\theta$ & $\rho$ &
\shortstack{pre-knee $K(n)$ slope\\obs./pred.} &
\shortstack{post-knee $K(n)$ slope\\obs./pred.}\\
\midrule
...
\centering
\small
\caption{Mean final excess risk at the best learning rate for each optimizer over 10 trials and 16,000 updates. Slightly negative $\qeff$ estimates are finite-window deviations consistent with the $\qeff=0$ class.}
...
\caption{Compute-optimal and AdamW phase diagrams. Left: the positive compute-risk exponent for hard-source $(a,b)=(3,1.4)$ and saturating $(1.5,1.4)$ settings; points show the hard-source sweep. Center: for $(a,b)=(3,1.4)$, $M_\star$ and $K_\star$ share exponent $1/(\alpha+1)$ by balancing the two source-tail terms, while $N_\star$ has exponent $\alpha/(\alpha+1)$. Right: for $(a,b,\theta)=(3,1.4,1)$, the AdamW exponent grows linearly with $s$ until $s_\star=0.6$, then plateaus at the no-decay exponent.}
\label{fig:compute}
\end{figure*}
...
\cite{bordelon2020spectrum,canatar2021spectral,bahri2024explaining,simon2024more}.
...
\cite{yang2021tensor,qiu2025hyperparameter}.
...
\cite{amari1998natural,martens2015optimizing,gupta2018shampoo,vyas2025soap}.
...
Random-feature, convex-neural-network, and neural-kernel models provide tractable descriptions of representation geometry
